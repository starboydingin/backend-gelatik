  ...
  group('Modules M-E s.d. M-H Unit & Widget Tests', () {
    ...
    test(
      ...
      final success = await notifier.submitKritikSaran(
        userId: 1,
        kritik: 'Aplikasi sudah bagus, namun mohon tambahkan notifikasi realtime.',
        saran: 'Integrasi dengan WhatsApp gateway untuk pengingat tenggat pengembalian.',
      );

      expect(success, isTrue);
      expect(container.read(kritikSaranProvider).successMessage,
          'Kritik & Saran berhasil dikirimkan!');
    });

    testWidgets('3. LayananInternetScreen renders bandwidth statistics and router cards',
        (tester) async {
      await tester.pumpWidget(
        const ProviderScope(
          child: MaterialApp(
            home: LayananInternetScreen(),
          ),
        ),
      );
    ...
    testWidgets('4. DaftarPegawaiScreen renders list of employees without email',
        (tester) async {
      await tester.pumpWidget(
        const ProviderScope(
          child: MaterialApp(
    ...
    testWidgets('6. KritikSaranScreen form submit shows confirmation dialog and closes',
      ...
      await tester.enterText(
          find.byType(TextField).at(0), 'Perlu peningkatan antarmuka.');
      await tester.enterText(
          find.byType(TextField).at(1), 'Tambahkan fitur dark mode otomatis.');
