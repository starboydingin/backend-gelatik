...
void main() {
  ...
  group('Panel Admin (M-L) Unit & Widget Tests', () {
    testWidgets('1. Login as Admin displays 4th tab "Admin" in AppBottomNav', (tester) async {
    ...
    testWidgets('2. Login as regular User DOES NOT display 4th tab "Admin"', (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            authProvider.overrideWith((ref) {
    ...
    test('3. Admin setujuPeminjaman updates status from "Menunggu" to "Proses" in provider', () async {
      ...
      final pendingItem = initialList.firstWhere((p) => p.status == 'Menunggu');

      final success = await notifier.setujuPeminjaman(pendingItem.id);

      final updatedState = container.read(peminjamanProvider);
      final updatedItem = updatedState.listPinjam.firstWhere((p) => p.id == pendingItem.id);

      expect(success, isTrue);
      expect(updatedItem.status, 'Proses');
    });

    testWidgets('4. Rejecting email proposal without entering notes triggers validation error', (tester) async {
    ...
    test('5. Admin setujuUsulanEmail updates status to "disetujui" and assigns emailResmi', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      ...
      final diajukanItem = initialList.firstWhere((e) => e.status == 'diajukan');
      ...
      final updatedItem = updatedState.listUsulanEmail.firstWhere((e) => e.id == diajukanItem.id);
    ...
    testWidgets('6. AdminPeminjamanDetailScreen: Rejecting loan without notes triggers error and prevents submit', (tester) async {
    ...
    testWidgets('7. AdminUsulanEmailDetailScreen: Approving email with empty emailResmi triggers error and prevents submit', (tester) async {
      const usulanDiajukan = UsulanEmailModel(
        id: 502,
        userId: 1,
        idPegBkd: 1,
